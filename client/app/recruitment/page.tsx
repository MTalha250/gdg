...
export default function RecruitmentPage() {
  ...
  function validateCurrentStep(): string[] {
    ...
    if (steps[step].key === "basic") {
      ...
      } else if (!/^bs[a-zA-Z0-9]{7}$/.test(values.rollNumber.toLowerCase())) {
        errors.push(
  ...
  async function submit() {
    ...
    try {
      ...
      const submissionData = {
        ...
        timeCommitmentAgreement: Boolean(values.timeCommitmentAgreement),
        attendanceCommitment: Boolean(values.attendanceCommitment),
        professionalismCommitment: Boolean(values.professionalismCommitment),
      };

      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/recruitment`,
        submissionData
      );
      toast.success(
